...
mixture <- function(x, data, k=length(x),
    ...
    EMstep <- function(p,all=FALSE) {        
        ...
        pp <- lapply(ParPos,function(x) thetacur0[x])
    ...
    if (optim$rerun) {
        opt <- tryCatch(ucminf(opt$par,myObj),error=function(x) opt)
    }
